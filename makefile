...
all:
	mkdir -p bin
	cd src && $(CC) $(CINCLUDES) $(CFLAGS) $(CLIBS) ogl_utils.cpp particles_gl.cpp container_gl.cpp world_gl.cpp mover_gl.cpp font_gl.cpp lodepng.c renderer.cpp glfw_utils.cpp controls.cpp setup.cpp hash_sort.cpp fluid.cpp communication.cpp -o ../bin/sph.out
clean:
	rm -f ./sph.out
	rm -f ./*.o
