...
void constructBoundaryBox(boundary_particle *boundary_particles, AABB* boundary, param *params)
{
    ...
    boundary_particles[i].x = min_x;
    boundary_particles[i].y = max_y;
    boundary_particles[i].z = min_z;
    boundary_particles[i].n_x = recip_root_three;
    boundary_particles[i].n_y = -recip_root_three;
    boundary_particles[i].n_z = recip_root_three;
    i++;
    // Corner front bottom right
    boundary_particles[i].x = max_x;
    boundary_particles[i].y = max_y;
    boundary_particles[i].z = min_z;
    boundary_particles[i].n_x = -recip_root_three;
    boundary_particles[i].n_y = -recip_root_three;
    ...
    for (nx=0; nx<num_x-2; nx++) {
        // Bottom right row
        boundary_particles[i].x = min_x + spacing + nx*spacing;
        boundary_particles[i].y = max_y;
        boundary_particles[i].z = min_z;
        boundary_particles[i].n_x = 0.0;
        boundary_particles[i].n_y = -recip_root_two;
        boundary_particles[i].n_z = recip_root_two;
        i++;
        // Top right row
        boundary_particles[i].x = min_x + spacing + nx*spacing;
        boundary_particles[i].y = max_y;
    ...
    for (ny=0; ny<num_y-2; ny++) {
        // Bottom front row
        boundary_particles[i].x = max_x;
        boundary_particles[i].y = min_y + spacing + ny*spacing;
        boundary_particles[i].z = min_z;
        boundary_particles[i].n_x = -recip_root_two;
        boundary_particles[i].n_y = 0.0;
        boundary_particles[i].n_z = recip_root_two;
        i++;
        // Top front row
        boundary_particles[i].x = max_x;
    ...
    for (nz=0; nz<num_z-2; nz++) {
        ...
        boundary_particles[i].x = min_x;
        boundary_particles[i].y = max_y;
        boundary_particles[i].z = min_z + spacing + nz*spacing;
        boundary_particles[i].n_x = recip_root_two;
        boundary_particles[i].n_y = -recip_root_two;
        boundary_particles[i].n_z = 0.0;
        i++;
        // right front column
        boundary_particles[i].x = max_x;
        boundary_particles[i].y = max_y;
